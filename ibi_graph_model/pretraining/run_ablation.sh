#!/bin/bash
# Submit one IBI graph pretraining job per loss function.
# Usage: bash run_ablation.sh

set -euo pipefail

PROJ=/work/nvme/bebr/mkhan14/ecg_foundation_model/ibi_graph_model
EXPERIMENTS=${PROJ}/experiments

LOSSES=(baseline huber vicreg barlow)

for LOSS in "${LOSSES[@]}"; do

    RUN_NAME="ibi_graph_${LOSS}"
    OUTPUT_DIR="${EXPERIMENTS}/${RUN_NAME}"
    mkdir -p "${OUTPUT_DIR}"

    sbatch <<SLURM
#!/bin/bash
#SBATCH --job-name=ibi_pretrain_${LOSS}
