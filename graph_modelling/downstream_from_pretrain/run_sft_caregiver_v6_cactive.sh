#!/bin/bash
#SBATCH --job-name=sft_care_v6_cact
...
EXPERIMENT_DIR="$PWD/brp_sft_experiments_caregiver/graph_byol_v4_mse_nodrop_v6_cactive"
mkdir -p "$EXPERIMENT_DIR"

# V6: c_active vs infant only — cleanest detectable signal
# c_passive and c_pick dropped; ~14:1 class imbalance handled by
# inverse-frequency weighted CrossEntropyLoss in train.py
python main.py \
    --output_dir     "$EXPERIMENT_DIR" \
    --train_csv      "../../BRP_train.csv" \
    --test_csv       "../../BRP_test.csv" \
    --encoder_ckpt   "$ENCODER_CKPT" \
    --dataset_type   caregiver_v6 \
    --num_classes    2 \
    --batch_size     32 \
    --window_sec     30 \
    --max_beats      80 \
    ...
    --seed           42 \
    --run_name       "sft_caregiver_v6_cactive_vs_infant"
