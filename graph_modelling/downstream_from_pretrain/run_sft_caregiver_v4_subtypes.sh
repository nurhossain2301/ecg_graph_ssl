#!/bin/bash
#SBATCH --job-name=sft_care_v4_sub
...
EXPERIMENT_DIR="$PWD/brp_sft_experiments_caregiver/graph_byol_v4_mse_nodrop_v4_subtypes"
mkdir -p "$EXPERIMENT_DIR"

# Dataset v4: 3-class sub-type labels — infant / c_active / c_passive
# c-pick excluded (too few samples, transient state)
# 30s windows; num_classes=3
python main.py \
    --output_dir     "$EXPERIMENT_DIR" \
    --train_csv      "../../BRP_train.csv" \
    --test_csv       "../../BRP_test.csv" \
    --encoder_ckpt   "$ENCODER_CKPT" \
    --dataset_type   caregiver_v4 \
    --num_classes    3 \
    --batch_size     32 \
    --window_sec     30 \
    --max_beats      80 \
    ...
    --seed           42 \
    --run_name       "sft_caregiver_v4_subtypes_30s"
