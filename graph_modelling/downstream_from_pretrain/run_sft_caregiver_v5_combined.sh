#!/bin/bash
#SBATCH --job-name=sft_care_v5_comb
...
EXPERIMENT_DIR="$PWD/brp_sft_experiments_caregiver/graph_byol_v4_mse_nodrop_v5_combined"
mkdir -p "$EXPERIMENT_DIR"

# V5 = V2 + V3: transition windows (+-15s) + session-norm + dual-window
# 30s windows; max_beats=80 per window -> 160-beat sequences
python main.py \
    --output_dir     "$EXPERIMENT_DIR" \
    --train_csv      "../../BRP_train.csv" \
    --test_csv       "../../BRP_test.csv" \
    --encoder_ckpt   "$ENCODER_CKPT" \
    --dataset_type   caregiver_v5 \
    --num_classes    2 \
    --batch_size     32 \
    --window_sec     30 \
    --max_beats      80 \
    ...
    --seed           42 \
    --run_name       "sft_caregiver_v5_transition_sessionnorm_dualwindow"
