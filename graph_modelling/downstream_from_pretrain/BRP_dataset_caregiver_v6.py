...
class BRPGraphDatasetCaregiverV6(Dataset):
    """
    c_active vs infant — binary, cleanest-signal version.

    Drops c_passive and c_pick entirely so the model only sees the
    interaction type (active caregiving touch) that V4 showed is actually
    detectable from infant ECG.

    Class imbalance (~14:1 infant:c_active) is handled by the weighted
    CrossEntropyLoss in train.py, which assigns inverse-frequency weights
    at training time.  No architecture change is needed.

    Labels:
        0  c_active — c-active* interactions only
        1  infant   — i-alone (crying/active alert on floor) + i-move on floor
    """
    ...
    def _parse_annotation(self, ann_path, tone_offset):
        ...
        for start, end, interaction in interaction_segments:
            ...
            # c-passive and c-pick are intentionally excluded
    ...
    def _prepare_index(self):
        print("Indexing BRP Caregiver V6 (c_active vs infant, imbalance handled via loss weights)...")
        ...
        # Report imbalance so it is visible in the job log
        from collections import Counter
        counts = Counter(lbl for _, _, lbl in self.samples)
        print(f"Total samples: {len(self.samples)}  |  class counts: {dict(counts)}")
