#!/bin/bash
#SBATCH --job-name=sft_care_v3_trans
...
EXPERIMENT_DIR="$PWD/brp_sft_experiments_caregiver/graph_byol_v4_mse_nodrop_v3_transition"
mkdir -p "$EXPERIMENT_DIR"

# Dataset v3: transition windows only (+-15s of annotation onset/offset)
# Binary caregiver vs infant; 30s windows
python main.py \
    --output_dir     "$EXPERIMENT_DIR" \
    --train_csv      "../../BRP_train.csv" \
    --test_csv       "../../BRP_test.csv" \
    --encoder_ckpt   "$ENCODER_CKPT" \
    --dataset_type   caregiver_v3 \
    --num_classes    2 \
    --batch_size     32 \
    --window_sec     30 \
    --max_beats      80 \
    ...
    --seed           42 \
    --run_name       "sft_caregiver_v3_transition_30s"
