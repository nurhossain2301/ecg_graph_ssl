...
class TemporalEncoder(nn.Module):
    ...
    def forward(self, x, valid_mask=None):
        # src_key_padding_mask: True = ignore (pad positions)
...
class ECGEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.gnn = nn.ModuleList([
            ResidualGATLayer(cfg.d_model, dropout=cfg.dropout)
...
class ECGBYOLModel(nn.Module):
    ...
    def masked_forward(self, beats, rr, node_mask, valid_mask):
        ...
        # Step 5: GNN refinement + final norm (mirrors ECGEncoder.forward)
    ...
    def forward(self, beats, rr, valid_mask, node_mask, beats1, rr1, valid1, beats2, rr2, valid2):
        target, recon, x = self.masked_forward(beats, rr, node_mask, valid_mask)
        p1, z2, p2, z1_target = self.byol_forward(beats1, rr1, valid1, beats2, rr2, valid2)
        return target, recon, x, p1, z2, p2, z1_target
    ...
    def byol_forward(self, beats1, rr1, valid1, beats2, rr2, valid2, return_attn=False):
        ...
        # Online path: view 2 (needed for symmetric loss)
        z2_online = self.online_encoder(beats2, rr2, valid2)
        if return_attn:
            z2_online, attn2 = self._pool(z2_online, valid2, return_attn=True)
        else:
            z2_online = self._pool(z2_online, valid2)
        ...
        with torch.no_grad():
            ...
            # Target path: view 1
            z1_target = self.target_encoder(beats1, rr1, valid1)
            z1_target = self._pool(z1_target, valid1)
            z1_target = self.target_projector(z1_target)

        if return_attn:
            return p1, z2, p2, z1_target, attn1, attn2

        return p1, z2, p2, z1_target