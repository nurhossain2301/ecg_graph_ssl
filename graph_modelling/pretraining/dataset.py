...
class ECGGraphBYOLDataset(Dataset):
    def __init__(
        ...
        for f in self.file_list:
            if not Path(f).is_file():
                warnings.warn(f"Skipping missing ECG file {f!r}")
                continue
            try:
                info = sf.info(f)
            except Exception as exc:
                warnings.warn(f"Skipping unreadable ECG file {f!r}: {exc}")
                continue
            self.file_info.append({"path": f, "frames": info.frames})

        if not self.file_info:
            raise RuntimeError("No readable ECG files found in file_list.")
    ...
    def _load_peaks(self, file_path, start):
        """
        Return R-peak sample indices local to the current window.
        Tries the pre-computed _peaks.npy sibling first; falls back to
        detect_r_peaks on the already-loaded waveform slice.
        The returned array may be empty — extract_beats handles that.
        """
        peaks_path = file_path.replace(".wav", "_peaks.npy")
        if os.path.exists(peaks_path):
            all_peaks = np.load(peaks_path)
            lo, hi = start, start + self.window_size
            return all_peaks[(all_peaks >= lo) & (all_peaks < hi)] - start
        return None   # signal: fall back in caller

    def _build_graph_inputs(self, ecg, file_path=None, start=0):
        if file_path is not None:
            r_peaks = self._load_peaks(file_path, start)
        else:
            r_peaks = None
        ...
        beats, rr = extract_beats(
            ...
            pre_ratio=self.cfg.pre_ratio,
            post_ratio=self.cfg.post_ratio,
            min_pre_ms=self.cfg.min_pre_ms,
            min_post_ms=self.cfg.min_post_ms,
            max_pre_ms=self.cfg.max_pre_ms,
            max_post_ms=self.cfg.max_post_ms,
        )
    ...
    def __getitem__(self, idx):
        ...
        beats, rr = self._build_graph_inputs(ecg, file_path=file_path, start=start)
...
class GraphBYOLCollator:
    ...
    def _pad_pack(self, beats_list, rr_list):
        ...
        max_L = max(x.shape[1] for x in beats_list)  # variable across heart rates
        B     = len(beats_list)

        beats      = torch.zeros(B, max_n, max_L)
        rr         = torch.zeros(B, max_n, 2)
        valid_mask = torch.zeros(B, max_n, dtype=torch.bool)

        for i in range(B):
            n = beats_list[i].shape[0]
            L = beats_list[i].shape[1]
            beats[i, :n, :L] = beats_list[i]   # zero-pad beat dim if L < max_L
            rr[i, :n]        = rr_list[i]
            valid_mask[i, :n] = True
